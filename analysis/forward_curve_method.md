...
**Historical list references use the corrected analytical history.** `load_on_demand`, `load_list` and `load_short_term` read `history.load_comparison_history`, which keeps raw observations intact while correcting the documented Azure card denominator and excluding the mislabeled Together on-demand product and retired static AWS Capacity Block references. Missing historical Together on-demand prices remain unknown; they are not reconstructed from preemptible prices.

**The manual Nebius committed list has its own validity gate.** `load_list` checks its verification date against the build's as-of date and the configured 45-day limit. A fresh scrape date does not refresh that manual verification. Expired values are withheld from `list_nebius`, current exports and interactive list comparisons; the verified date and exclusion appear in provenance. The separately dated Finance grid and contract economics are unchanged. Regenerating the marks-history CSV normalizes line endings to LF while retaining prior dated rows.
